...
function build() {
	...
	echo "Publishing"
	"${DOTNET_BIN}" msbuild /nologo /t:CFPPublish
} # }}}

# FUNCTION: downloadAppBinary {{{
# Fetches Dotnet zip DLLs from a binary storage
#
...
function downloadAppBinary() {
	...
	if [[ "${success}" == "true" ]]; then
		...
		"${UNZIP_BIN}" -zxf "${destination}" -d "${outputDir}"
...
function retrieveGroupId() {
	"${DOTNET_BIN}" msbuild /nologo /t:CFPGroupId | xargs
} # }}}
...
function retrieveAppName() {
	if [[ "${PROJECT_NAME}" != "" && "${PROJECT_NAME}" != "${DEFAULT_PROJECT_NAME}" ]]; then
		...
		"${DOTNET_BIN}" msbuild /nologo /t:CFPAppName | xargs
	fi
} # }}}
...
function retrieveStubRunnerIds() {
	"${DOTNET_BIN}" msbuild /nologo /t:CFPStubIds | xargs
} # }}}
